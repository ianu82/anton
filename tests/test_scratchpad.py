...
class TestScratchpadInstall:
    ...
    async def test_install_packages_logs_telemetry(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(scratchpad_module, "log_package_event", lambda event, **kwargs: events.append((event, kwargs)))
        monkeypatch.setattr(scratchpad_module.asyncio, "create_subprocess_exec", fake_create_subprocess_exec)
