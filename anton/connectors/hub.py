...
from anton.connectors.base import ConnectorClient, ConnectorError, QueryResult
...
class ConnectorHub:
    ...
    async def schema(self, connector_id: str) -> str:
        schema = await self._client.describe_schema(connector_id)
    ...
    async def query(self, connector_id: str, query: str, *, limit: int = 1000) -> str:
        result = await self._client.run_query(connector_id, query, limit=limit)
        return format_query_result(result)

    async def sample(self, connector_id: str, table: str, *, limit: int = 100) -> str:
        result = await self._client.sample(connector_id, table, limit=limit)
        return format_query_result(result)

    async def write(self, connector_id: str, query: str) -> str:
        result = await self._client.write(connector_id, query)
        return format_query_result(result)
