...
SCRATCHPAD_TOOL = {
    ...
    "description": (
        ...
        "full machine access.\n"
        "If Anton reports that `get_llm()` is available in this session, it returns a "
        "pre-configured LLM client (sync) — call llm.complete(system=..., messages=[...]) "
        "for AI-powered computation.\n"
        "If available, llm.generate_object(MyModel, system=..., messages=[...]) extracts "
        "structured data into Pydantic models. Supports single models and list[Model].\n"
        "If Anton reports that `agentic_loop()` is available in this session, it runs a "
        "tool-call loop where the LLM reasons and calls your tools iteratively. "
        "handle_tool(name, inputs) -> str is a plain sync function.\n"
        "Built-in secret-backed helpers such as `get_llm()` and `agentic_loop()` are "
        "mode-dependent. `query_minds_data()` is available whenever Anton reports a Minds "
        "datasource is configured, including restricted execution modes where Anton "
        "brokers the request for you.\n"
